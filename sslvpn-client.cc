...
extern "C"
{
    ...
    // 初始化虚拟网卡
    static int initTun(int global, char *svip, char *cvip, char *ipv4_net, unsigned int mtu);
...
#ifdef __APPLE__
                    /**
                     * utun 驱动需要明确说明，IP协议类型是AF_INET、还是AF_INET6，就是读入的IP包、写入utun的IP包头前有四个字节来表示
                     * 所以结构就是（注意：苹果是小端机，低位在前，高位在后，但是网络字节序是大端，就编程高位在前，低位在后）
                     *   所以就变成这样了：
                     *   0 0 0 0 IP头 IP载荷
                     */
                    unsigned char *chunk = packet + RECORD_HEADER_LEN - sizeof(uint32_t);
                    // TODO 实际需要判断是ipv4还是ipv6
                    *(uint32_t *)chunk = htonl(AF_INET);
                    int wlen = write(tun_fd, chunk, datalen + sizeof(uint32_t));
#else
    ...
    static void *client_tun_thread(void *arg)
    {
        ...
        while (1)
        {
             ...
             *   0 0 0 0 IP头 IP载荷
             */
            unsigned char *ip_packet = buf + 4;
#else
            unsigned char *ip_packet = buf;
#endif
            // 2、分析报文
            unsigned char src_ip[4];
            unsigned char dst_ip[4];
            memcpy(src_ip, &ip_packet[16], 4);
            memcpy(dst_ip, &ip_packet[20], 4);
            // printf("read tun data: %d.%d.%d.%d -> %d.%d.%d.%d (%d)\n", dst_ip[0], dst_ip[1], dst_ip[2], dst_ip[3],
            //        src_ip[0], src_ip[1], src_ip[2], src_ip[3], ret_length);

            // 3、对数据进行封包
            enpack_len = sizeof(packet);
            enpack(RECORD_TYPE_DATA, ip_packet, ret_length, packet, &enpack_len);

            // 4、直接发送到服务端
            int len = SSL_write(ssl, packet, enpack_len);
            if (len <= 0)
            {
                printf("SSL写入数据失败! 错误码：%d, 错误信息：'%s'\n", errno, strerror(errno));
                // TODO 需要重新发送
            }
    ...
    static int controlHandler(unsigned char *data, unsigned int len)
    {
        int ret = 0;
        cJSON *root = NULL;
        cJSON *iterm_global = NULL, *iterm_svip = NULL, *iterm_cvip = NULL, *iterm_ipv4_net = NULL, *iterm_mtu = NULL;
        ...
        if (memcmp(data, RECORD_TYPE_CONTROL_TUN_CONFIG, RECORD_TYPE_LABEL_LEN) == 0) // 虚拟网卡控制类型
        {
            ...
            iterm_svip = cJSON_GetObjectItemCaseSensitive(root, "svip");
            ...
            if (iterm_global == NULL || iterm_svip == NULL || iterm_cvip == NULL || iterm_ipv4_net == NULL || iterm_mtu == NULL)
            {
                printf("iterm is NULL\n");
                ret = -1;
                goto end;
            }
            ret = initTun(iterm_global->valueint, iterm_svip->valuestring, iterm_cvip->valuestring, iterm_ipv4_net->valuestring, iterm_mtu->valueint);
        }
